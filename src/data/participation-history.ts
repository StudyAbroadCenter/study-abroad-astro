...
/**
 * Verified headline KPIs from the same integrated workbook previously used by
 * the Global Learning Network / globe review build.
 *
 * Scope: RSJP, RSJP Express, RWJP and RWJP Express, 2023–2026.
 * Custom Programs are not included in these counts.
 */
export const shortTermParticipationEvidence = {
  period: '2023–2026',
  participants: 522,
  countriesOrRegions: 27,
  institutions: 114,
...
/**
 * Programme-specific institution evidence used by the RWJP / RWJP Express UI.
 * These are 2023–2026 totals aggregated from the verified workbook.
 * No participant names or other personal information are stored here.
 */
...
export function summarizeParticipation(records: ProgrammeParticipationRecord[]) {
  return {
    participants: records.reduce((sum, record) => sum + record.participants, 0),
    countries: new Set(records.map((record) => record.countryOrRegion)).size,
    institutions: new Set(records.map((record) => `${record.countryOrRegion}\u0000${record.institution}`)).size,
  };
}
...
export function participationByCountry(records: ProgrammeParticipationRecord[]) {
  ...
  records.forEach((record) => {
    const country = grouped.get(record.countryOrRegion) ?? { participants: 0, institutions: new Map<string, number>() };
    country.participants += record.participants;
    country.institutions.set(record.institution, (country.institutions.get(record.institution) ?? 0) + record.participants);
    grouped.set(record.countryOrRegion, country);
  });

  return [...grouped.entries()]
    .map(([countryOrRegion, value]) => ({
      countryOrRegion,
      participants: value.participants,
      institutions: [...value.institutions.entries()]
        .map(([institution, participants]) => ({ institution, participants }))
        .sort((a, b) => b.participants - a.participants || a.institution.localeCompare(b.institution)),
    }))
    .sort((a, b) => b.participants - a.participants || a.countryOrRegion.localeCompare(b.countryOrRegion));
}

export const participationHistoryStatus = {
  status: 'verified' as const,
  source: participationHistorySource,
  note: 'Recovered from the previously supplied integrated workbook and reconciled to the former globe evidence layer.',
};
