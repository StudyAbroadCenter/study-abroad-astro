...
const cn: Record<ChineseProgrammeId, LocalPage> = {
  ...
  rwjp: {
    ...
    overviewTitle: '从课堂到京都生活，连续五周学习与实践', overviewText: '零基础可以申请，但开始前必须能读写平假名和片假名；JLPT N1持有者不符合申请条件。',
    academicTitle: '系统日语课程＋文化学习', academicIntro: '日语学习57.5小时，文化学习990分钟。', academicItems: ['日语课程57.5小时。', '文化学习990分钟。', '在校园和京都生活中实践所学。'],
    eligibilityTitle: '申请前请确认基本资格', eligibilityItems: ['整个项目期间须为大学在籍学生。', '零基础可申请，但开始前须能读写平假名和片假名。', 'JLPT N1持有者不可申请。'],
    accommodationTitle: '可选择大将军宿舍', accommodationText: `宿舍费${yen(rdf)}，入住${fmt(offers.rwjp.checkIn)}，退房${fmt(offers.rwjp.checkOut)}。`,
    facts: [{ label: '项目日期', value: range(rd?.start, rd?.end), status: '已确认' }, { label: '申请期', value: range(ra?.start, ra?.end), status: '已确认' }, { label: '项目费用', value: yen(rf), status: '已确认' }, { label: '日语学习', value: '57.5小时', status: '已确认' }, { label: '文化学习', value: '990分钟', status: '已确认' }, { label: '宿舍费', value: yen(rdf), status: '已确认' }, { label: '付款后退款扣除', value: offers.rwjp.refundDeductionJPY === null ? '待确认' : `${yen(offers.rwjp.refundDeductionJPY)}不可退还的行政手续费；其余金额退还`, status: offers.rwjp.refundDeductionJPY === null ? '待确认' : '已确认' }],
    nextTitle: `准备申请${offers.rwjp.academicYear} RWJP？`, nextText: '请再次确认资格、申请期、费用、住宿和最新官方说明。',
  },
  'rwjp-express': {
    family: '日语与日本文化', title: `${offers['rwjp-express'].academicYear} RWJP Express｜立命馆大学`, description: `${offers['rwjp-express'].academicYear} RWJP Express设有1月OIC场次与2月衣笠场次。`,
    heroAlt: '参加RWJP Express短期日语项目的学生', kicker: `${offers['rwjp-express'].academicYear} · Winter Express · 日语与日本文化`, headline: '用约两周时间，集中体验日语学习与日本大学生活。',
    ...
    eligibilityTitle: '面向大学与研究生院在籍学生', eligibilityItems: ['整个项目期间须为大学在籍学生。', '日语零基础申请者须在开始前能读写平假名和片假名。', 'JLPT N1持有者不符合申请条件。'],
    accommodationTitle: '可使用大将军宿舍', accommodationText: '原则上多人入住，房间分配由大学决定。',
    facts: [{ label: '项目日期', value: range(e.dates?.start, e.dates?.end), status: '已确认' }, { label: '最低开班人数', value: offers['rwjp-express'].minimumParticipants === null ? '待确认' : `${offers['rwjp-express'].minimumParticipants}人`, status: offers['rwjp-express'].minimumParticipants === null ? '待确认' : '已确认' }, { label: '日语学习', value: '22.5小时', status: '已确认' }, { label: '文化学习', value: '360分钟', status: '已确认' }, { label: '付款后退款扣除', value: offers['rwjp-express'].refundDeductionJPY === null ? '待确认' : `${yen(offers['rwjp-express'].refundDeductionJPY)}不可退还的行政手续费；其余金额退还`, status: offers['rwjp-express'].refundDeductionJPY === null ? '待确认' : '已确认' }],
...
const tw: Record<ChineseProgrammeId, LocalPage> = {
  rsjp: {
    ...
    heroAlt: '參加立命館大學短期日語課程的學生', kicker: `${offers.rsjp.academicYear} Summer · 日語與日本文化`, headline: '在京都學日語，把課堂所學帶進真實生活。', lead: 'RSJP以日語學習為核心，連結校園生活、文化體驗與京都日常。', campus: '1月：大阪茨木校區（OIC） · 2月：衣笠校區（京都）', status: 'pending', statusLabel: `${offers.rsjp.academicYear}年度詳細資訊待確認`, statusNote: '課程名稱與校區已確認；年度日期、費用、申請期間與住宿只在正式確認後公布。',
  ...
  rwjp: {
    family: '日語與日本文化', title: `${offers.rwjp.academicYear} RWJP｜立命館大學冬季日語課程`, description: `${offers.rwjp.academicYear} RWJP於京都衣笠校區舉行，日期為${range(rd?.start, rd?.end, '待確認')}。`, heroAlt: '參加立命館大學冬季日語課程的學生', kicker: `${offers.rwjp.academicYear} · Winter · 日語與日本文化`, headline: '在冬季京都，用五週時間深入學習日語。', lead: 'RWJP以日語學習為核心，連結文化學習、校園生活與京都日常中的實際使用。', campus: '衣笠校區 · 京都', status: 'confirmed', statusLabel: `${offers.rwjp.academicYear}核心資訊已確認`, statusNote: `日期、申請期間、費用、學習時數、住宿與主要條件已確認。申請期間為${range(ra?.start, ra?.end, '待確認')}。`, overviewTitle: '從教室到京都生活，連續五週學習與實踐', overviewText: '零基礎可以申請，但開始前必須能讀寫平假名與片假名；JLPT N1持有者不符合申請資格。', academicTitle: '系統化日語課程＋文化學習', academicIntro: '日語學習57.5小時，文化學習990分鐘。', academicItems: ['日語課程57.5小時。', '文化學習990分鐘。', '在校園與京都生活中實際使用所學。'], eligibilityTitle: '申請前請確認基本資格', eligibilityItems: ['整個課程期間須為大學在籍學生。', '零基礎可申請，但開始前須能讀寫平假名與片假名。', 'JLPT N1持有者不可申請。'], accommodationTitle: '可選擇大將軍宿舍', accommodationText: `宿舍費${yen(rdf, '待確認')}，入住${fmt(offers.rwjp.checkIn, '待確認')}，退房${fmt(offers.rwjp.checkOut, '待確認')}。`, facts: [{ label: '課程日期', value: range(rd?.start, rd?.end, '待確認'), status: '已確認' }, { label: '申請期間', value: range(ra?.start, ra?.end, '待確認'), status: '已確認' }, { label: '課程費用', value: yen(rf, '待確認'), status: '已確認' }, { label: '日語學習', value: '57.5小時', status: '已確認' }, { label: '文化學習', value: '990分鐘', status: '已確認' }, { label: '宿舍費', value: yen(rdf, '待確認'), status: '已確認' }, { label: '付款後退費扣除', value: offers.rwjp.refundDeductionJPY === null ? '待確認' : `${yen(offers.rwjp.refundDeductionJPY, '待確認')}不可退還的行政手續費；其餘金額退還`, status: offers.rwjp.refundDeductionJPY === null ? '待確認' : '已確認' }], nextTitle: `準備申請${offers.rwjp.academicYear} RWJP？`, nextText: '請再次確認資格、申請期間、費用、住宿與最新官方說明。',
  },
  'rwjp-express': {
    family: '日語與日本文化', title: `${offers['rwjp-express'].academicYear} RWJP Express｜立命館大學`, description: `${offers['rwjp-express'].academicYear} RWJP Express設有1月OIC場次與2月衣笠場次。`, heroAlt: '參加RWJP Express短期日語課程的學生', kicker: `${offers['rwjp-express'].academicYear} · Winter Express · 日語與日本文化`, headline: '用約兩週時間，集中體驗日語學習與日本大學生活。', lead: '密集日語課程、文化學習與校園體驗集中在約兩週內。', campus: '衣笠校區 · 京都', status: 'confirmed', statusLabel: `${offers['rwjp-express'].academicYear}主要資訊已確認`, statusNote: '日期、最低開班人數、學習時數、住宿基本條件與退費扣除規則已確認。', overviewTitle: '短期、集中、以日語學習為核心', overviewText: '適合希望在較短時間內集中學習日語並體驗京都校園生活的學生。', academicTitle: '22.5小時日語＋360分鐘文化學習', academicIntro: '日語課程1,350分鐘，文化學習360分鐘。', academicItems: ['日語課程1,350分鐘。', '文化學習360分鐘。', '結合校園與京都體驗。'], eligibilityTitle: '以大學與研究所在學學生為對象', eligibilityItems: ['整個課程期間須為大學在籍學生。', '日語零基礎申請者須在開始前能讀寫平假名與片假名。', 'JLPT N1持有者不符合申請資格。'], accommodationTitle: '可使用大將軍宿舍', accommodationText: '原則上多人入住，房間配置由大學決定。', facts: [{ label: '課程日期', value: range(e.dates?.start, e.dates?.end, '待確認'), status: '已確認' }, { label: '最低開班人數', value: offers['rwjp-express'].minimumParticipants === null ? '待確認' : `${offers['rwjp-express'].minimumParticipants}人`, status: offers['rwjp-express'].minimumParticipants === null ? '待確認' : '已確認' }, { label: '日語學習', value: '22.5小時', status: '已確認' }, { label: '文化學習', value: '360分鐘', status: '已確認' }, { label: '付款後退費扣除', value: offers['rwjp-express'].refundDeductionJPY === null ? '待確認' : `${yen(offers['rwjp-express'].refundDeductionJPY, '待確認')}不可退還的行政手續費；其餘金額退還`, status: offers['rwjp-express'].refundDeductionJPY === null ? '待確認' : '已確認' }], nextTitle: `想參加${offers['rwjp-express'].academicYear} RWJP Express？`, nextText: '申請前請確認最終招生簡章與住宿資訊。',
  },
  rdsp: {
