...
for (const relativePath of presentationFiles) {
  ...
  for (const literal of forbidden) {
    if (content.includes(literal)) violations.push({ file: relativePath, literal });
  }
}
...
console.log(`Offering fact audit passed across ${presentationFiles.length} presentation surfaces: ${dateValues.length} dates and ${moneyValues.length} monetary values protected.`);
