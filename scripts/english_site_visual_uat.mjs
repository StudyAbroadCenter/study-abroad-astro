...
for (const viewport of viewports) {
  for (const route of routes) {
    ...
    page.on('response', (response) => {
      ...
      try { if (new URL(response.url()).origin === new URL(baseUrl).origin) localResourceErrors.push(`${response.status()} ${response.url()}`); } catch {}
    });
    ...
    await page.evaluate(async () => {
      ...
      for (let y = 0; y < document.documentElement.scrollHeight; y += Math.max(480, Math.floor(innerHeight * 0.75))) {
        scrollTo(0, y);
      ...
      await Promise.race([
        Promise.all(images.map(async (img) => {
          ...
          if (img.naturalWidth && img.decode) { try { await img.decode(); } catch {} }
        })),
    ...
    const metrics = await page.evaluate(({ routeId, required, forbidden }) => {
      ...
      const smallTargets = innerWidth <= 390 ? interactive.map((element) => { const rect = element.getBoundingClientRect(); return { text: (element.textContent || '').trim().slice(0, 60), width: Math.round(rect.width), height: Math.round(rect.height) }; }).filter((item) => item.width < 44 && item.height < 44) : [];
      const textNodes = [...document.querySelectorAll('main p, main li, main dd, main a, main small')].filter((element) => { const style = getComputedStyle(element); return style.display !== 'none' && style.visibility !== 'hidden'; });
      const tooSmall = textNodes.map((element) => ({ text: (element.textContent || '').trim().slice(0, 70), size: Number.parseFloat(getComputedStyle(element).fontSize) })).filter((item) => item.text && item.size < 11.5);
      const detailLinks = routeId === 'home' ? [...document.querySelectorAll('[data-en-programme-link]')].map((element) => ({ id: element.getAttribute('data-en-programme-link') || '', href: element.getAttribute('href') || '', rect: { width: Math.round(element.getBoundingClientRect().width), height: Math.round(element.getBoundingClientRect().height) } })) : [];
      return {
        ...
        requiredMissing: required.filter((text) => !mainText.includes(text)),
        forbiddenPresent: forbidden.filter((text) => mainText.includes(text)),
        ...
        tooSmall,
        detailLinks,
        japaneseLeak: [...document.querySelectorAll('main *')].map((element) => element.textContent || '').find((text) => /[ぁ-んァ-ン一-龯]/.test(text)) || '',
      };
    }, { routeId: route.id, required: route.required, forbidden: route.forbidden });

    const expectedHomeLinks = ['/en/programs/rsjp/', '/en/programs/rsjp-express/', '/en/programs/rwjp/', '/en/programs/rwjp-express/', '/en/programs/rdsp/', '/en/programs/rbmp/'];
    const homeLinkPaths = metrics.detailLinks.map((link) => link.href);
    const homeLinksMissing = route.id === 'home' ? expectedHomeLinks.filter((expected) => !homeLinkPaths.some((href) => href.includes(expected))) : [];
    const homeSmallLinks = route.id === 'home' && viewport.width <= 390 ? metrics.detailLinks.filter((link) => link.rect.width < 44 && link.rect.height < 44) : [];
    const status = response?.status() ?? 0;
    const routeFailed = status >= 400 || metrics.lang !== 'en' || !metrics.robots.includes('noindex') || !metrics.canonical.includes(route.path) || !metrics.jaAlternate || metrics.h1Count !== 1 || metrics.headingSkips > 0 || metrics.textLength < route.minText || metrics.overflowX > 2 || metrics.requiredMissing.length > 0 || metrics.forbiddenPresent.length > 0 || metrics.unloadedImages.length > 0 || metrics.smallTargets.length > 0 || metrics.tooSmall.length > 0 || Boolean(metrics.japaneseLeak) || homeLinksMissing.length > 0 || homeSmallLinks.length > 0 || pageErrors.length > 0 || consoleErrors.length > 0 || localResourceErrors.length > 0;
    ...
    results.push({ viewport: viewport.name, route: route.path, id: route.id, httpStatus: status, ...metrics, homeLinksMissing, homeSmallLinks, pageErrors, consoleErrors, localResourceErrors, screenshot, passed: !routeFailed });
    await context.close();
  }
}

await browser.close();
const summary = { generatedAt: new Date().toISOString(), total: results.length, passed: results.filter((item) => item.passed).length, failed: results.filter((item) => !item.passed).length, routes: routes.map((route) => route.path), viewports: viewports.map((viewport) => viewport.name), results };
await fs.writeFile(path.join(outputDir, 'report.json'), JSON.stringify(summary, null, 2));
for (const item of results) console.log(`${item.passed ? 'PASS' : 'FAIL'} ${item.viewport} ${item.route} status=${item.httpStatus} overflow=${item.overflowX}px missing=${item.requiredMissing.length} forbidden=${item.forbiddenPresent.length} smallTargets=${item.smallTargets.length} tiny=${item.tooSmall.length} images=${item.unloadedImages.length}`);
if (failed) { console.error('English site-wide UAT failed. Inspect english-site-visual-uat/report.json and screenshots.'); process.exit(1); }
