...
for(const viewport of viewports){
  ...
  const routeFailed=status>=400||metrics.lang!=='en'||metrics.bodyText<500||metrics.overflowX>2||!metrics.hero||!metrics.finder||Boolean(metrics.japaneseLeak)||metrics.unloadedImages.length>0||pageErrors.length>0||consoleErrors.length>0||localResourceErrors.length>0;
...
for(const item of report)console.log(`${item.passed?'PASS':'FAIL'} ${item.viewport} /en/ status=${item.httpStatus} overflowX=${item.overflowX}px lang=${item.lang} unloadedImages=${item.unloadedImages.length}`);
